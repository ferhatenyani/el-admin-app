...
const SearchAnalytics = () => {
  ...
  // Recent searches table - independent pagination
  ...
  useEffect(() => {
    ...
    const fetchAnalytics = async () => {
      try {
        setIsLoading(true);
        const [statsData, trendData, topData, zeroData] = await Promise.all([
          searchAnalyticsApi.getSearchStats(days),
          searchAnalyticsApi.getSearchTrend(days),
          searchAnalyticsApi.getTopTerms(days, 10),
          searchAnalyticsApi.getZeroResultTerms(days, 10),
        ]);
        if (cancelled) return;
        setStats(statsData);
        setTrend(fillTrendGaps(trendData, days));
        setTopTerms(topData);
        setZeroTerms(zeroData);
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const fetchRecent = async () => {
      try {
        ...
        const page = await searchAnalyticsApi.getRecentSearches(recentPage, 10);
        if (!cancelled) setRecent(page);
      } catch (error) {
